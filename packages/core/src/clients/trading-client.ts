...
import type { TradingCredentials } from '../types/index.js';
...
export class TradingClient {
  ...
  constructor(credentials: TradingCredentials) {
    this.provider = new JsonRpcProvider(credentials.polygonRpcUrl);
    this.proxyAddress = credentials.gnosisSafeAddress;
    this.dryRun = credentials.dryRun;

    // Create wallet connected to provider
    this.wallet = new Wallet(credentials.masterPrivateKey, this.provider);

    const creds: ApiKeyCreds = {
      key: credentials.clobApiKey,
      secret: credentials.clobSecret,
      passphrase: credentials.clobPassphrase,
    };
